...
// Project:            SPAR
// Authors:            ni24039
// Description:        Unit test for the subscribe script
//
// Modifications:
...
#define BOOST_TEST_MODULE SubscribeScriptTest
...
BOOST_AUTO_TEST_CASE(SubscribeScriptWorks) {
  ...
  subscription_data.AddLine(Knot(new string("0 0 lname = 'ADAMS'")));
  subscription_data.AddLine(Knot(new string("1 10 fname = 'DONNA'")));

  SubscribeScript script(subscription_data, &protocol_stack, &logger);
  boost::thread script_thread(
      boost::bind(&SubscribeScript::Run, &script));

  boost::this_thread::sleep(boost::posix_time::milliseconds(100));

  VerifyIStreamContents(list_of(string("COMMAND 0"))("SUBSCRIBE 0")
    ("lname = 'ADAMS'")("ENDCOMMAND"), sut_input_streams[0]);
  ...
  *sut_output_streams[0] << "RESULTS 0\n"
  ...
  VerifyIStreamContents(list_of(string("COMMAND 1"))("SUBSCRIBE 10")
    ("fname = 'DONNA'")("ENDCOMMAND"), sut_input_streams[1]);

...
performer='blr'                # Needs to match the general naming convention of
                               # muddler and test-gen files
testname='sample-mixed'
...
sleep 1;
cd $runner_dir
script -c "python remote_runner.py -c ${performer}_config.py -m ${performer}-${testname}_muddler.py -u $username -p SPARtest -l DEBUG" $archive_dir/$testname/rr_log.txt

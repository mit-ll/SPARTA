...
// Project:             SPAR
// Authors:             ni24039
// Description:         PublishingServer for TA3.1 baseline.
//
// Modifications:
 ...
 * @see PubSubActor
 */
public class PublishingServer extends PublishingActor {
    /** Logger for this class. */
    private static final Logger LOGGER =
        LoggerFactory.getLogger(PublishingServer.class);
     ...
     */
    public PublishingServer(final LineRawReader in,
                            final Writer out,
    ...
    /** Silly constructor required by StubbedPublishingServer in test. */
    protected PublishingServer() {
      output = null;
    }
    ...
    // CHECKSTYLE:ON
        assert session != null : "Attempting to publish before "
          + "PublishingServer has been started.";
        String[] metadataValues = null;
        try {
          metadataValues = schemaHandler.splitMetadata(metadata);
        } catch (ProtocolException e) {
            return detailedExceptionString("PublishingServer failed to parse"
        ...
        try {
            ...
            return detailedExceptionString("PublishingServer failed to create"
                + " text message(s).\n", e);
        } catch (ProtocolException e) {
            return detailedExceptionString("PublishingServer failed to parse"
                    + " publish metadata values.\n", e);
        ...
        for (TextMessage msg : messages) {
          try {
              ...
              return detailedExceptionString("PublishingServer failed to "
                      + "publish message.\n", e);
          }
        }
        return null;
  ...
  public static void main(final String[] args) {
    ...
    PublishingServer pub = null;
    try {
      pub = new PublishingServer(
      ...
      LOGGER.error(
        detailedExceptionString("Could not instantiate PublishingServer.\n", e));
      System.exit(1);
    }
    final PublishingServer finalPub = pub;
    ...
    try {
      ...
      LOGGER.error(
        detailedExceptionString("Error occurred when running PublishingServer.\n", e));
      System.exit(1);
    }
    try {
      finalPub.shutdown();
